...
//     const addBtn = categoriCard.querySelector(".addBtn");
//     addBtn.addEventListener("click", () => {
//       alert(`${InnrCat.name} has been added to the card`);
...
//         <div>
//           <h2 class="font-semibold">${InnrCat.name}</h2>
//           <p class="text-sm text-gray-600">৳${InnrCat.price}</p>
//         </div>
//         <div>
...
//       total += parseFloat(InnrCat.price);
...
//     addBtn.addEventListener("click", () => {
//       alert(`${allPlant.name} has been added to the card`);
//       const cartDisplay = document.getElementById("cartsBtn");
//       const cartContainer = document.createElement("div");
//       cartContainer.classList.add(
//         "flex",
...
const displyCatData = (CatDatas) => {
  ...
  for (let CatData of CatDatas) {
    ...
    CatBtn.classList.add("space-y-4", "cursor-pointer");

    CatBtn.innerHTML = `
      <button onclick="InnrCat(${CatData.id}, this)" 
        class="w-full text-left px-3 py-1 rounded hover:bg-[#15803d] hover:text-white cursor-pointer">
...
const displyInnrCat = (InnrCats) => {
  ...
  for (let InnrCat of InnrCats) {
    ...
    categoriCard.innerHTML = `
      ...
      <button class="modalBtn font-semibold px-3 mt-2 cursor-pointer">${InnrCat.name}</button>
      <p class="text-sm p-3 text-gray-600">${InnrCat.description}</p>
      <div class="flex justify-between items-center px-3 mt-2">
        <span class="text-xs px-3 py-1 bg-green-100 text-green-700 rounded">${InnrCat.category}</span>
        <span class="font-semibold">৳${InnrCat.price}</span>
      </div>
      <div class="p-3">
        <button class="addBtn w-full mt-3 mx-auto py-2 rounded-full bg-green-600 text-white hover:bg-green-700 cursor-pointer">
          Add to Cart
        </button>
      </div>
    `;

    const modalBtn = categoriCard.querySelector(".modalBtn");
    ...
    const addBtn = categoriCard.querySelector(".addBtn");
    addBtn.addEventListener("click", () => {
      alert(`${InnrCat.name} has been added to the cart`);
      const cartDisplay = document.getElementById("cartsBtn");
      const cartContainer = document.createElement("div");
      cartContainer.classList.add(
        "flex",
...
const showModal = (plant) => {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-lg w-11/12 max-w-lg p-4 relative">
         <h2 id="modalName" class="text-xl font-semibold my-4"></h2>
        <img id="modalImage" class="w-full h-64 object-cover rounded" src="" alt="">
       
        <p class="text-sm text-gray-600 my-5" id="modalDesc"></p>
        <div class="flex justify-between mt-2">
          <span class="text-xs px-2 py-1 bg-green-100 mb-16 rounded" id="modalCategory"></span>
          <span class="font-semibold " id="modalPrice"></span>
        </div>
        <button id="closeModal" class="btn absolute bottom-2 right-2 text-gray-700 font-bold text-lg">close</button>
      </div>
    `;
  ...
  modal.querySelector("#modalImage").src = plant.image;
  modal.querySelector("#modalName").textContent = plant.name;
  modal.querySelector("#modalDesc").textContent = plant.description;
  modal.querySelector("#modalCategory").textContent = plant.category;
  modal.querySelector("#modalPrice").textContent = `Price:৳ ${plant.price}`;
...
const allPlantsDisply = (allPlants) => {
  ...
  for (let allPlant of allPlants) {
    ...
    plantsCard.innerHTML = `
      ...
      <button class="modalBtn font-semibold px-3 mt-2 cursor-pointer">${allPlant.name}</button>
      <p class="text-sm p-3 text-gray-600">${allPlant.description}</p>
      <div class="flex justify-between items-center px-3 mt-2">
        <span class="text-xs px-3 py-1 bg-green-100 text-green-700 rounded">${allPlant.category}</span>
        <span class="font-semibold">৳${allPlant.price}</span>
    ...
    const modalBtn = plantsCard.querySelector(".modalBtn");
    modalBtn.addEventListener("click", () => {
      showModal(allPlant);
    });
    ...
    const addBtn = plantsCard.querySelector(".addBtn");
    addBtn.addEventListener("click", () => {
      alert(`${allPlant.name} has been added to the cart`);
      const cartDisplay = document.getElementById("cartsBtn");
      const cartContainer = document.createElement("div");
      cartContainer.classList.add(
        "flex",
